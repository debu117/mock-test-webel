...
import { resultPageStyles, resultPageAnimations,
    getColorClass,
    colorClasses,
    getBadgeClasses,
    getBadgeText
...
const MyResult = () => {
  ...
  const { headingTitle } = useMemo(() => {
    ...
    let headingTitle = "Test Results";
    if (tracks.length === 1) {
      headingTitle = `Test Results — ${tracks[0]} Track`;
    } else if (tracks.length === 2) {
      headingTitle = `Test Results — ${tracks.join(" & ")} Tracks`;
    } else if (tracks.length > 2) {
      headingTitle = `Test Results — Multiple Tracks`;
    }
    return { headingTitle };
  }, [results]);
